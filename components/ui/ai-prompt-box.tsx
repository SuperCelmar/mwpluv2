...
// Embedded CSS for minimal custom styles with theme support
const styles = `
  *:focus-visible {
...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  ...
  // Determine if dark mode is active (resolvedTheme already handles system theme)
  ...
  const handleInputChange = (value: string) => {
    setInput(value);
    if (enableAddressAutocomplete && onAddressInputChange) {
      onAddressInputChange(value);
    }
  };

  const handleAddressSuggestionClick = (address: any) => {
    if (onAddressSelect) {
      onAddressSelect(address);
      setInput(address.properties.label);
    }
  };
      ...
      {mounted && (
        <div className="flex justify-center mb-6">
          <Image
            ...
            width={200}
            height={60}
            className="h-auto w-auto max-w-[200px] transition-opacity duration-300"
            priority
          />
        </div>
      )}
