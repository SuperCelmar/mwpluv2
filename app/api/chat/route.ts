...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[CHAT_API] Prepared webhook payload:', {
      hasDocumentId: !!webhookPayload.document_id,
      documentIdsCount: webhookPayload.document_ids?.length || 0,
      hasGps: !!webhookPayload.gps_coordinates,
      hasContext: !!webhookPayload.context_metadata,
    });
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        console.log(`[CHAT_API] Sending webhook request (attempt ${attempt})`);
        ...
        if (webhookResponse.ok) {
          ...
          const responseMessage =
            webhookData?.message || webhookData?.response || 'Réponse reçue';

          console.log('[CHAT_API] Webhook succeeded');
